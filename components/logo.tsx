...
export const Logo = ({
  ...
  const gradientId = `cohub-logo-gradient-${useId()}`;
  return (
    <svg
      viewBox="0 0 140 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={cn("text-foreground h-7 w-auto md:h-8", className)}
      role="img"
      aria-labelledby={`title-${gradientId}`}
    >
      <title id={`title-${gradientId}`}>CoHub</title>

      {/* Monogram mark (simple "C" with hub dot) */}
      <g transform="translate(0,3)">
        <path
          d="M12.5 1.5a7 7 0 1 0 0 14"
          fill="none"
          stroke={uniColor ? "currentColor" : `url(#${gradientId})`}
          strokeWidth="2.2"
          strokeLinecap="round"
        />
        <circle
...
export const LogoIcon = ({
  ...
  const gradientId = `cohub-icon-gradient-${useId()}`;
  return (
    <svg
      width="18"
      height="18"
      ...
      aria-labelledby={`title-${gradientId}`}
    >
      <title id={`title-${gradientId}`}>CoHub icon</title>
      <path
        d="M12.5 4.5a5 5 0 1 0 0 9"
        fill="none"
        stroke={uniColor ? "currentColor" : `url(#${gradientId})`}
        strokeWidth="2"
        strokeLinecap="round"
